...
| 9 | Мониторинг: совместимое расширение контракта v1 — `chain` в `/state`, `?edge=` в `/probe/configs`, path `edge:<name>` | §6 |
...
| состояние | `pending` (заведено, join-токен выдан, бокс ещё не вошёл) → `joined`; плюс `legacy` для мигрированного host override (2.3) |
...
`chainRevision` — состояние, а не предпочтение: в `entity.AllSetting` его **нет**, чтобы сохранение формы настроек не откатило ревизию (та же логика, что у `monLastContact`, `web/service/setting.go:96-99`). `chainExtraPorts`, `chainPollSeconds`, `chainStaleMinutes`, `chainJoinTokenHours` — предпочтения, они в `AllSetting` и в форме.
...
| `POST /panel/api/chain/del/:id` | удалить звено (§4.5) |
...
| `GET /panel/api/chain/edges/health` | здоровье edge-звеньев по данным мониторинга (§6.4) |
...
"chain": {"revision": 42, "activeEdge": "edge-a",
          "edges": [{"name":"edge-a","host":"a.example.net","state":"joined"},
                    {"name":"edge-b","host":"b.example.net","state":"pending"}]}
```

Плюс `GET /mon/v1/probe/configs?edge=<name>` и ключ `path` вида `edge:<name>`. Запись в реестр снаружи в v1 не открывается: весь ввод — через `ChainService` (единственный путь записи, он же держит инварианты и `chainRevision`). Будущий оркестратор получит `POST /mon/v1/chain/*` поверх того же сервиса — это и есть шов, отдельного слоя под него не строим.

### 2.5 Telegram

Команда `/proxy` уже зарегистрирована (`web/service/tgbot.go:294`, `case "proxy"` — `tgbot.go:733`); меняется только её тело:

- `/proxy` без аргументов — список звеньев: роль, имя, хост, состояние (`joined`/`pending`/`legacy`), свежесть (`lastRevision` против `chainRevision`), здоровье от мониторинга (UP/DOWN/UNKNOWN по `path = edge:<name>`), маркер активного. Ключ `tgbot.commands.chainList` + `tgbot.commands.chainHop`.
- `/proxy <имя>` — переключить активный edge: запись в реестр, `chainRevision++`, ответ `tgbot.commands.chainSwitched` (`Name==`, `Host==`). Неизвестное имя, `pending`-звено или inner → тот же список плюс `tgbot.commands.chainUnknownName`.
...
Регистрация в `SetMyCommands` не меняется, меняется только `tgbot.commands.proxyDesc` (`translate.en_US.toml:1155`) → «Show and switch the proxy chain edges (admin)». Новые ключи добавляются **в конец** секции `tgbot.commands` во всех 13 локалях; начинаем с `en_US`/`ru_RU` (отсутствующий ключ падает в имя ключа). Защита от пустого описания (`tgbot.go:282-287`) остаётся.
...
**2.6.2 Два запасных edge.** Три edge подвешены к `inner[n]` (или к панели), у всех `next_hop_id` одинаков, `is_active` — у одного. `inner[n]` держит в своём документе `secretHash` всех трёх и впускает всех: standby-edge тоже качает подписки и relay'ит — иначе переключение было бы не мгновенным. Переключение (`setActive` или `/proxy <имя>`) — одна запись в реестре плюс `chainRevision++`; на боксах не меняется **ничего** (§3.5). Мониторинг зондирует все edge, активный и запасные (`path = edge:<name>`).
...
1. Активных edge не больше одного, и он в состоянии `joined` или `legacy` (`pending`-звено активным сделать нельзя).
2. `next_hop_id` любого edge = последний inner в состоянии `joined`/`legacy`, либо `NULL`, если таких inner'ов нет.
3. Inner'ы образуют **один путь**: ровно один inner с `next_hop_id = NULL`, у каждого следующего `next_hop_id` — предыдущий, `position` подряд от 0 без дыр, ветвлений нет.
...
1. Хост звена владелец вводит руками; панель его не проверяет и не резолвит. Нужен ли в UI кнопочный «проверить» (TCP-коннект на `subPort` из панели)? Это разовый исходящий запрос панели на бокс — формально нарушение «панель наружу не звонит», но по инициативе человека.
2. `legacy`-звено после миграции живёт до переустановки бокса. Ограничивать ли его жизнь (баннер через N дней, отказ заводить новые звенья, пока есть `legacy`)?
...
{"version":1,"revision":42,"generatedAt":1758380000000,
 "self":{"name":"inner-2","role":"inner","host":"203.0.113.9"},
...
- `self` — как звено называется в реестре. Имя нужно ему для `/chain/v1/status` и логов, `host` — чтобы заметить расхождение с тем, что владелец ввёл (§4.4).
...
- `hops` — **сам звено и всё, что снаружи от него**; `secretHash` = `sha256(hop secret)` hex. Звено использует из этого списка только хэши своих **прямых** внешних соседей (тех, кто к нему подключается); остальные записи — материал для UI и для сверки `outer[]`.
...
```json
{"version":1,"revision":42,"generatedAt":1758380000000,
 "self":{"name":"edge-a","role":"edge","host":"a.example.net"},
...
- Интервал опроса — `chainPollSeconds` (default 30), с джиттером ±20 %, чтобы цепочка не опрашивалась синхронной волной.
...
Секретов и списка `hops` в статусе нет. Неверный bearer — `404`.
...
1. **xray inbounds** — из сгенерированного конфига xray, с теми же правилами пропуска, что у `proxy/relay.go:49 skipReason`. Пропускаются ровно пять случаев:
   - `port <= 0` — «no port»;
...
`x-ui chain ports` на панели — **отладочный** экспорт этого же списка; `x-ui relay-manifest` остаётся deprecated-алиасом (§5.8). Вручную его больше никуда не вставляют.
...
| `main.go:576` | `chain ports` + алиас `relay-manifest`; новые подкоманды `chain status`/`chain rejoin`/`chain join-url` (§5.8) |

### Открытые вопросы владельцу

1. `chainPollSeconds = 30` даёт до 30 с × длину цепочки на прокатывание волны до края (три звена ≈ 1,5 мин). Приемлемо, или нужен «пинок» — уменьшение интервала до 5 с на 2 минуты после любого изменения ревизии (звено, увидев новую ревизию, ускоряется само и заражает этим внешних соседей)?
2. Sub-порт звена обслуживает и подписки, и волну. Разносить ли их по разным портам, чтобы волну можно было закрыть файрволом отдельно от публичных подписок, или одного порта достаточно (решение карты: inner'ы открыты всем, без allowlist)?
...
| срок | `chainJoinTokenHours`, default 24 ч; `join_token_expires` — абсолютное время в мс |
...
`POST /panel/api/chain/del/:id`, в одной транзакции:

1. Проверить 2.6.4 (активный edge удаляется только через `force` и только последним).
2. Внешнего соседа удаляемого звена перецепить на его next hop: `outer.next_hop_id = deleted.next_hop_id`; для удаляемого последнего inner'а — перецепить **все** висящие на нём edge.
3. Сжать `position` у оставшихся inner'ов.
4. Удалить строку, `chainRevision++`.

Волна доносит новый `nextHop` до внешнего соседа за ≤ `chainPollSeconds`, он перезапускает relay и начинает ходить мимо удалённого.

**Нужно ли мягкое удаление.** Отдельного состояния `draining` в v1 **не вводим**. Рассуждение: запись в реестр мгновенна, но реального разрыва клиентов она не вызывает — удалённый бокс продолжает relay'ить, пока жив, и ровно этим держит старые соединения. Опасен не сам delete, а слишком раннее выключение бокса владельцем. А эту проблему мягкое удаление и не решает: панель не гасит боксы, гасит их человек или провайдер.

Поэтому минимальный безопасный ответ — не состояние, а **подсказка**: ответ `del` несёт `{"safeToPowerOffWhen":{"hop":"edge-a","revision":43}}`, а в реестре у внешнего соседа и так видно `lastRevision`. UI показывает «звено удалено; подождите, пока `edge-a` покажет ревизию ≥ 43, затем выключайте бокс» и сам меняет плашку, когда подтверждение приходит. Стоимость — ноль новых состояний и ноль новых инвариантов; `draining` со всеми его крайними случаями (сосед мёртв и никогда не подтвердит; таймаут; что делать с `draining`-звеном при новом удалении) ввести всегда успеем, если практика покажет, что владельцы гасят боксы слишком рано.
...
1. **В панели:** удалить мёртвое звено из реестра (`del`). Реестр перецепляет внешнего соседа на следующее внутрь звено и бампает ревизию. Панель знает всю топологию, поэтому шаг тривиален.
...
| `docs/runbooks/proxy-front.md` | разделы «Вход звена», «Смерть inner'а», «Удаление звена» |

### Открытые вопросы владельцу

1. TTL join-токена 24 ч: хватает ли на «завёл звено вечером, поставил бокс через два дня»? Альтернатива — default 72 ч, но тогда просроченный неиспользованный токен дольше висит боевым.
2. `x-ui chain rejoin` меняет next hop бокса по одной команде с ssh-доступом. Требовать ли подтверждения (`--yes`) или предварительного `x-ui chain status`, чтобы уменьшить шанс перецепить не тот бокс в стрессовой ситуации?
...
| `x-ui chain join-url [-c <config>]` | **новая, замена** `proxy-setup-url` | печатает URL join page или «this box has already joined the chain» и `exit 1` |
| `x-ui proxy-setup-url` | алиас, deprecated | делает то же, но первой строкой в stderr: `proxy-setup-url is deprecated, use: x-ui chain join-url` |
| `x-ui chain status [-c <config>]` | новая | ходит на свой `GET /chain/v1/status` по `127.0.0.1:<subPort>` и печатает: имя, роль, next hop, ревизию, `stale`, relayed ports, время последней успешной волны |
| `x-ui chain rejoin --next-hop <host> [--sub-port <n>] --token <t> [-c <config>]` | новая | переписывает `nextHop`, обнуляет `hopSecret`, выполняет join немедленно (без страницы) и перезапускает релей; так runbook чинит цепочку после гибели inner front |
| `x-ui chain ports [-o <file>]` | **на панели**, новая | debug-экспорт вычисленного списка relayed ports (`port`, `network`, `tag`, `source`) — того самого, что уходит в документ цепочки |
| `x-ui relay-manifest [-o <file>]` | алиас, deprecated | печатает то же, что `chain ports`, со строкой `relay-manifest is deprecated, use: x-ui chain ports`; manifest-формат больше не производится |

> **Решение об имени:** команда `relay-manifest` остаётся, но основным именем становится `x-ui chain ports` — «manifest» после отмены манифеста читается как обещание файла, который некуда вставлять; алиас сохраняет совместимость со скриптами и runbook'ом.
...
На стенде это: панель остаётся real server, нынешний `proxy` становится **inner front** (или сразу edge, если цепочка из одного звена).

1. На панели: Settings → Subscription → *Chain* → создать звено (`name`, `role`, `next hop`), получить **join token** (TTL 24 ч, одноразовый). Для inner front next hop — сама панель.
...
1. **TTL join-токена и повторное использование.** 24 ч и одноразовость — дефолт §4.1. Нужен ли «долгоиграющий» токен для массового разворачивания standby edge (например, `n` использований)?
2. **`x-ui chain rejoin` без токена.** Сейчас rejoin требует свежий токен из реестра. Альтернатива — разрешить re-join по действующему `hopSecret` (звено уже известно панели, меняется только next hop). Второе удобнее при гибели inner front, но хранит на боксе долгоживущий пропуск в цепочку.
3. **Судьба `proxy-setup-url` и `relay-manifest`.** Держим алиасы вечно или удаляем через релиз после того, как все боксы переустановлены?
4. **Что делает бокс, если документ протух дольше `staleMinutes` и сосед не вернулся.** Сейчас: релеит по старой ревизии бесконечно. Нужен ли верхний предел (например, сутки), после которого звено само уходит в bootstrap и показывает join page?
5. **Порядок для edge с TLS.** Join page на `subPort` с `cert`/`key`: если сертификат ещё не выпущен, страница поднимается по HTTP на том же порту — приемлемо ли отдавать join-токен по HTTP, или требовать `PROXY_JOIN_TOKEN` для таких боксов?
...
```json
{
  ...
  "chain": {
    ...
    "activeEdge": "ams-1",
    "edges": [
      {"name": "ams-1", "host": "203.0.113.10", "state": "joined"},
      {"name": "ams-2", "host": "203.0.113.20", "state": "joined"},
      {"name": "fra-1", "host": "203.0.113.30", "state": "pending"}
    ]
  }
}
```
...
- `edges` — только edge-звенья реестра (`role=edge`), в порядке `name` asc. `state` = состояние звена в реестре (`pending`/`joined`/`legacy`); inner-звенья в контракт мониторинга не попадают (§6.3). `host` у `pending` — адрес, введённый владельцем при создании звена (join ещё не подтвердил его), поэтому такие edge не пробируются.
...
{"override": {"enabled": true, "host": "front.example.net"},
 ...
 "chain": {"activeEdge": "ams-1",
           "edges": [{"name":"ams-1","host":"203.0.113.10","state":"joined"}, "…"]}}
```

- `chain.edges` отсортированы по `name`, входят только `joined`/`legacy` (звенья, которые реально можно опрашивать; `pending` не влияет на targets, поэтому не входит в хэш — вход в реестр без подтверждения join не должен пересобирать targets).
- `activeEdge` — **в хэше**, а не только в `edges`: `proxy`-алиас указывает на активное edge, и когда host override переключается между уже известными edge (без изменения списка `edges`), targets `path=proxy` (для старого mon-server) и `path=edge:<active>` (де-факто тот же адрес) должны пересобраться — адрес поменялся, а `edges` как список нет.
- Переименование edge не входит в хэш само по себе, только если меняется набор/состав/host/state/activeEdge — рифмуется с тем, что `remark`/`tag` inbound'а не входят в хэш (переименование не двигает targets), но у edge переименование **меняет** `path` (`edge:<name>`), поэтому по факту потянет пересборку, т.к. `name` — часть ключа объекта `edges[]`.

#### `GET /probe/configs?edge=<name>`

Третий режим ручки — наравне с «без параметров» (path `proxy`, через host override) и `?host=` (path `direct`):

- `GET /probe/configs?edge=<name>` — рендерит конфиги probe-набора с адресом **этого** edge вместо host override, независимо от того, активно оно или нет. `host`, порт, SNI, ключи — как для `direct`/`proxy`, меняется только адрес.
- `409 unknown_edge` — имени нет в реестре edge-звеньев.
- `409 edge_not_joined` — имя есть, но звено в состоянии `pending` (join не завершён, relay на нём ещё не поднят; для `legacy` ручка работает — это донный edge миграции, эквивалент старого `proxyOverrideHost`).
- `?host=` и поведение без параметров — не меняются (см. §6.3 про то, что даёт `proxy` при наличии цепочки).

#### `path`

Три значения вместо двух: `direct`, `proxy` (алиас = active edge, оставлен для старых mon-server), `edge:<name>` (новое; `<name>` — `[a-z0-9-]{1,32}`, как имя звена в реестре). Ключ результата `(monClientId, kind, inboundId, path)` не меняется по форме — просто у `path` больше допустимых значений. `/events` и `/stats` принимают `edge:<name>` как обычную строку `path`, без валидации по реестру (панель не хранит topology мониторинга — она пассивный приёмник, ADR 0003): неизвестное имя edge в `path` события — не ошибка, просто ещё одна строка в ленте. Словарь `reason` не меняется — деградация edge выглядит так же, как деградация proxy front сегодня (`tcp_refused`, `tls_timeout`, …).

#### Удаление и переименование edge

- **Удаление** edge из реестра — каскадное удаление хранимого состояния по префиксу path: все строки `mon_targets`/`mon_events`/`mon_stats_current`/`mon_stats_rollup` с `path = 'edge:<name>'` или (для активного на момент удаления) `path = 'proxy'`, если это имя было active edge, удаляются в той же транзакции, что и удаление hop (по образцу каскада для удалённого inbound, §3.6 monitoring-panel.md). Telegram молчит — как и при удалении inbound.
- **Переименование** — новое имя звена = новый `path` (`edge:<new-name>`); отдельного переезда строк нет. Старые строки под `edge:<old-name>` доживают обычный ретеншн (`monRetentionDays`/`monRollupRetentionDays`) и стареют сами; мгновенного каскада на переименование нет — оно того же рода событие, что появление нового edge, только с историей под старым именем, которая сама истечёт за неделю/месяц.
...
- На каждый `GET /state` с изменившейся ревизией (или изменившимся `chain.revision`) mon-server читает `chain.edges`; для каждого звена в состоянии `joined` или `legacy` зовёт `GET /probe/configs?edge=<name>` (host = хост этого звена) — как сегодня зовёт `?host=<real>` для `direct`. `pending`-звенья пропускаются (не отвечают — `409 edge_not_joined`, join ещё не завершён).
- **Набор targets** = inbound'ы × (`{direct}` ∪ `{joined/legacy edges}`), т.е. на inbound теперь `2 + N` target'ов вместо двух (`direct`, `proxy`), где `N` — число edge в цепочке (обычно 1 активный + K запасных).
- **Распределение по mon-client**: без изменений — все mon-client пробуют все edge и `direct`, как сегодня пробуют оба path; фильтра «этот mon-client только на этот edge» в v1 нет (тот же принцип, что «все inbound'ы всем mon-client» в mon-protocol.md §4.2). `paths` каждого mon-client в admin UI mon-server по-прежнему список строк, просто теперь длиннее (`direct`, `edge:ams-1`, `edge:ams-2`, …) — коробки во враждебных регионах владелец по-прежнему ограничивает нужным подмножеством.
- **`proxy` как отдельный target**: mon-server, знающий про цепочку (v2 протокола mon-server↔mon-client-конфига, не путать с контрактом панели — версия контракта не растёт), **не** держит `path=proxy` отдельным target'ом — он разворачивает его в `edge:<active>` и меняет target при каждом переключении active edge. Старый mon-server (не читающий `chain`) продолжает получать `proxy` через ручку без параметров и не узнает про остальные edge — деградация до текущего поведения, не поломка.
...
### 6.3 Пробинг inner-звеньев

**Решение: нет, не в v1.** Inner front не видят клиенты, у них нет входа для трафика с клиентскими протоколами (xray/AWG inbound'ов там не заводится) — пробовать нечего тем же механизмом probe account. Мониторинг проверяет цепочку с точки зрения клиента: `direct` (реальный адрес, для владельца) и каждое edge (то, что видит клиент). Здоровье relay внутри цепочки (inner ↔ inner, inner ↔ real server) — задача wave/heartbeat самой цепочки (poll `chain.v1/document`, `last_seen_at` в реестре), не задача mon-server.

> Задел на будущее (не проектируем, только называем): `path=inner:<name>` — прямая проверка relay-сегмента без прохождения всей цепочки, для диагностики «где именно рвётся» без гадания по разнице `direct` vs `edge:*`. Отдельный тикет после того, как появится нужда.

### 6.4 Панель: UI

#### Бейдж на звено в редакторе цепочки

`web/html/settings/panel/subscription/chain.html`: у каждого edge-звена (не у inner) — бейдж состояния, худший `target` по всем inbound'ам с `path = edge:<name>` (для active edge — c учётом алиаса `proxy`, т.е. свёртка по обоим `path`), та же свёртка и приоритет, что у колонки Health (`DOWN > FLAPPING > UNKNOWN > UP > PAUSED`; `STALE` панели перекрывает всё). Для `pending`-звена бейдж не про здоровье, а «нет данных» (серый, отдельная метка, не путать с `UNKNOWN` target'а) — mon-server туда ещё не ходил.

- Метод `MonitoringService.WorstLiveTargetState` обобщается до `WorstLiveTargetState(inboundKind, inboundId, pathPrefix string)` (пустой `pathPrefix` = как сегодня, все path); новый метод-обёртка `MonitoringService.WorstLiveEdgeState(edgeName string) string` — сворачивает по всем inbound'ам сразу для одного edge (для бейджа в редакторе не нужна раскладка по inbound).
- Новая ручка `GET /panel/api/chain/edges/health` (сессия, конверт `{success,msg,obj}`) в контроллере реестра цепочки (`web/controller/chain_controller.go`, файл из рамки §11) — отдаёт `[{name, state}]` для всех edge реестра одним запросом на страницу редактора; переиспользует `WorstLiveEdgeState`, дополнительных таблиц не заводит.

#### Страница Monitoring

`web/html/monitoring.html`:

- Группировка/фильтр по `path` — вместо статичных `direct`/`proxy` строится динамически по `GET /panel/api/chain/edges/health` (список имён edge) + фильтр-чипы `direct`, `edge:<name>` на каждое edge, `all`; активному edge — маркер (та же звёздочка/цвет, что у active edge в редакторе). Существующая ручка `GET targets` (§7.4 monitoring-panel.md) уже отдаёт `path` каждой строки — новых полей от неё не требуется, фильтрация клиентская, как сегодня фильтр `down` у Health.
- Строка-сводка над таблицами: «активный edge DOWN, запасное ams-2 UP» — считается на клиенте из уже загруженных `targets` + `chain` (активное имя и список edge панель уже знает из реестра — тот же вызов, что у редактора, `GET /panel/api/chain/edges/health`, дергается и здесь). Формула — как в §6.5 (свёртка «здоров ли edge»): для active edge берётся его состояние, для остальных — лучший из standby.
- Изменённые ручки: `GET targets` без изменений формы ответа (значения `path` — новые строки, схема та же). Новая: `GET /panel/api/chain/edges/health` (переиспользуется и редактором, и страницей Monitoring — одна ручка, два потребителя).
...
- **Список запасных** — все joined/legacy edge, кроме active, с их сегодняшним состоянием (`WorstLiveEdgeState`, свёртка по всем inbound'ам, тем же способом, что бейдж в редакторе).
- **Выбор кандидата на переключение** — по приоритету: все inbound'ы `UP` (полностью здоров) > большинство inbound'ов `UP` (частично) > `UNKNOWN`/нет данных; `DOWN` кандидатом не рассматривается. При равенстве — по имени (`asc`). Если кандидатов нет вовсе (все standby `DOWN` или список standby пуст) — строка «нет здорового запасного» вместо «Переключить: …».
...
Реализация — **после мержа эпика мониторинга #43** и тикетов реестра и волны этой карты (без реестра `chain.edges` неоткуда взять). Тикеты, которые встанут в очередь на этот раздел (заводятся отдельно, здесь только перечислены как зависящие):

1. Контракт: поле `chain` в `GET /state`, режим `?edge=` у `GET /probe/configs`, каскад удаления/старение переименования — правки `monitoring_service.go`, `monitoring.go` (контроллер), `monitoring_ingest.go` не нужен (path не валидируется).
2. mon-server (репо `3ax-ui-monitoring`): чтение `chain.edges`, построение targets `edge:<name>`, разворачивание `proxy` в `edge:<active>` для v2-конфигов mon-client.
3. Панель UI: бейдж в `chain.html`, ручка `/panel/api/chain/edges/health`, группировка/сводка на `monitoring.html`.
4. Telegram: подсказка о запасных в алерте DOWN активного edge, кнопка «Сделать активным».
5. (Задел, не тикет сейчас) `path=inner:<name>` — если понадобится диагностика relay-сегментов.

### Тронутые файлы

**Репо `SBKubric/3ax-ui-proxy` (панель):**
- `web/service/monitoring_service.go` — `State()` отдаёт `chain`, ревизия учитывает `chain.edges`/`activeEdge`; `ProbeConfigs(edge)`; обобщение `WorstLiveTargetState` + новый `WorstLiveEdgeState`; каскад удаления hop по `path`-префиксу.
- `web/controller/monitoring.go` — `GET /probe/configs` разбирает `?edge=`, коды `409 unknown_edge`/`409 edge_not_joined`.
- `web/controller/chain_controller.go` — новая ручка `GET /panel/api/chain/edges/health` (файл и группа `/panel/api/chain/...` — §2.4, здесь только добавляется один хендлер).
...
- `docs/spec/mon-protocol.md`, `docs/spec/mon-server.md` — построение targets по `chain.edges`, разворачивание `proxy` → `edge:<active>`.
- Код mon-server: цикл `GET /state` → чтение `chain` → пересборка targets (модуль, аналогичный сегодняшнему построению `direct`/`proxy`).
- admin UI mon-server: `paths` per-mon-client теперь список из `direct` + динамических `edge:<name>` вместо статичной пары.

### Открытые вопросы владельцу

1. Название и точный путь ручек реестра цепочки (`chain_controller.go`, группа `/panel/api/chain/...`) фиксирует эпик #43 — этот раздел предполагает, что там будет чтение списка hop'ов и переключение active edge; если имена лягут иначе, `GET /panel/api/chain/edges/health` и кнопка «Сделать активным» переезжают на них без смысловых изменений.
2. Порог «большинство inbound'ов UP» для выбора кандидата в Telegram-подсказке (§6.5) — не определён численно (что при 2 inbound'ах: 1 из 2 — большинство или ничья?); нужно решение владельца или дефолт «>50%, ничья не считается большинством».
3. Ретеншн для строк `edge:<old-name>` после переименования — тот же `monRetentionDays`/`monRollupRetentionDays`, что у обычных данных, или короче (раз это заведомо мёртвый path)? Предложение — не заводить отдельный параметр, оставить общий ретеншн.
...
Модалка «Add hop» (`chain-add-hop-modal`): поля Name (`chain-add-hop-name`), Host (`chain-add-hop-host`), Role — select inner/edge (`chain-add-hop-role`), Next hop — select real-server или любой inner (`chain-add-hop-next`).

Удаление — `a-modal-confirm`; попытка удалить active edge вместо диалога подтверждения показывает предупреждение `pages.settings.chain.deleteActiveRefused` («Choose another edge with "Make active" first, then delete this one.») и не удаляет звено.
...
| `chain-add-hop-modal` / `-name` / `-host` / `-role` / `-next` | модалка и её поля |
...
Контроллер `web/controller/chain_controller.go`, группа `/panel/api/chain/` (сессия, конверт `{success,msg,obj}`), маршруты из §2.4: `GET list`, `POST add` (ответ несёт join-токен один раз), `POST update/:id`, `POST del/:id` (активный edge — отказ `active_edge_in_use`, клиент показывает `chain.deleteActiveRefused`), `POST setActive/:id` (только `joined`/`legacy` edge), `POST reissueToken/:id`, `GET edges/health` (§6.4). Не путать с `/chain/v1/*` на sub-сервере (§3.3, §4.3), которыми пользуются сами звенья.

### 7.4 Бейджи здоровья

Переиспользуют классы страницы мониторинга (`web/html/monitoring.html`, `mon-state mon-state-<state>`, стили в `custom.min.css`): `mon-state-up` (зелёный), `mon-state-down` (красный), `mon-state-unknown`/`mon-state-none` (серый). Источник значения — `GET /panel/api/monitoring/state` §6 (`chain.edges[].health`, поле добавляется тем же тикетом реестра); для inner-звеньев в MVP здоровья нет (мониторинг проб не идёт глубже edge, см. рамку — «мониторинг каждого таргета через каждый edge»), бейдж стоит `UNKNOWN` до отдельного тикета.
...
| `state.pending` / `state.joined` / `state.legacy` | pending / joined / legacy | pending / joined / legacy |
...
Существующие `tgbot.commands.proxyStatus`/`proxyEnabled`/`proxyDisabled`/`proxyUsage` (одиночный override) не удаляются в этом тикете — реализация `/proxy` на цепочке (§7.6) их заменит в отдельном тикете реестра; сейчас это только текст для мок-вывода.
...
1. Реестровый тикет заменит текущий одиночный `/proxy <host|off>` (override, `tgbot.commands.proxyStatus` и соседние) на цепочечный `/proxy [name]` — оставляем старые ключи для обратной совместимости команды на переходный период или удаляем сразу вместе с миграцией `proxyOverrideHost` → `edge`-hop (§2)?
2. Для inner-звеньев в макете бейдж здоровья всегда `UNKNOWN` (мониторинг не пробирует inner). Нужен ли отдельный сигнал «inner видит свой next hop» (например, `last_seen_at` из реестра, §2) хотя бы как текстовая подсказка в UI, или это отдельный тикет?
...
| 8 | Релиз и стенд | релиз-тег панели и бокса; e2e на стенде `real ← proxy (inner) ← edge` по §10; закрытие [#76](https://github.com/SBKubric/3ax-ui-proxy/issues/76) — предпосылка | 4, 5, 7 |
...
- **Компрометация edge.** Изъятый бокс знает свой hop secret, адрес и sub-порт next hop, список relayed ports и своё имя (§3.7). Ротация: удалить звено из реестра (§4.5) — его секрет мёртв на следующем же документе соседа; при необходимости перевыпустить токен и переустановить бокс на новом хосте. Общего секрета, который нужно было бы ротировать по всей цепочке, нет.
...
## 10. E2E на стенде `real ← proxy (inner) ← edge`

Предпосылка: [#76](https://github.com/SBKubric/3ax-ui-proxy/issues/76) — третий VPS `edge`. Стенд из карты #2: `real` (панель), `proxy` (нынешний front, становится inner).

1. Обновить панель на `real` до релиза; в реестре появилось `legacy`-звено `edge` с хостом `proxy` — host override работает как раньше (§2.3).
2. Завести в реестре inner `inner-1` (host = `proxy`), переустановить `proxy` как звено по §5.10 с `PROXY_JOIN_TOKEN`; `x-ui chain status` → `joined`; `legacy`-звено удалить.
3. Завести edge `edge-a` (next hop вычислился = `inner-1`), поставить бокс `edge` через join page (без `PROXY_JOIN_TOKEN`) — проверка второго пути входа; сделать активным через `/proxy edge-a`.
4. Клиент: подписка `/sub/<id>` через `edge` отдаёт адрес `edge`, VLESS через `edge → proxy → real` ходит, AWG UDP 51820 ходит, `Profile-Web-Page-Url` указывает на `edge`.
5. На панели добавить inbound на новом порту → за ≤ 2 × `chainPollSeconds` порт открыт на `proxy` и `edge` (`ss -ltnup`); удалить — закрылся.
6. Завести второй edge `edge-b` на том же VPS `edge` невозможно (один хост — одно звено), поэтому standby проверяется как pending-звено: токен выдан, в списке `/proxy` видно `pending`, `setActive` на него отказывает.
7. `/proxy off` → override выключен, подписки отдают адрес `real`; `/proxy edge-a` → обратно.
8. Удалить `inner-1` из реестра (`del`) → `edge` за ≤ `chainPollSeconds` перецепился на `real` напрямую (`x-ui chain status` → `nextHop.host = real`), трафик ходит; вернуть `inner-1` через вставку (§2.6.3) и `chain rejoin` на `edge` не требуется — `edge` перецепляется волной.
9. Погасить `proxy` (имитация смерти inner) → `edge` через `chainStaleMinutes` в `stale`; починка по §4.6 (`del` + `reissueToken` + `x-ui chain rejoin` на `edge`).
10. `grep -R "privateKey\|password" /etc/x-ui /usr/local/x-ui/bin` на обоих боксах пуст; `update.sh` на боксе с v2-конфигом проходит, на сохранённом v1-конфиге — останавливается до подмены бинаря (§5.7).
