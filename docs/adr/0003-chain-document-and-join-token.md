...
- `x-ui relay-manifest` is demoted to a debug export of the computed port list (renamed `x-ui chain ports`, old name kept as a deprecated alias); `x-ui proxy-setup-url` becomes `x-ui chain join-url`, likewise aliased.
