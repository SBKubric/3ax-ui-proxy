...
| `PROXY_FRONT` | `off` | `only443` ставит перед звеном nginx на 443: TCP только на 443, разводка по SNI и файрвол (см. ниже) |
...
**Фронт 443 на звене** (`PROXY_FRONT=only443` или `"front": {"mode": "only443"}` в `/etc/x-ui/proxy.json`): nginx занимает 443 и разводит его по SNI. Edge пропускает имя сервера своего target-соседа сырым потоком на next hop, а незнакомое имя — сырым потоком на сам target-сосед; inner пропускает имя active edge, остальным отдаёт заглушку; запрос по IP без SNI попадает в подписки звена и `/chain/v1` через IP-сертификат. Relay перестаёт держать 443/tcp и прочие TCP-порты (UDP релеится как раньше), а звено закрывает всё, кроме 443/tcp, 80/tcp, SSH и relayed UDP (`"firewall": false` оставляет порты в покое). Звено сообщает о фронте при каждом опросе, панель переводит его sub-порт на 443, а старый sub-порт закрывается, когда опрашивающие звено соседи переехали; edge закрывает его сразу, и клиентские ссылки подписки становятся `https://<edge>/…`. Нужен IP-сертификат Let's Encrypt — без него фронт не поднимается. Подробно: `docs/runbooks/proxy-front.md` §3.5.
