...
1. На реальной панели скопируйте её xray-конфиг (`/usr/local/x-ui/bin/config.json`) на прокси-бокс (например, в `/root/panel-config.json`) — релей берёт из него порты инбаундов.
2. На прокси-боксе запустите установщик в режиме прокси:
...
(или запустите `install.sh` в терминале и ответьте на вопрос **«Install as PROXY FRONT?»**). Настройки лежат в `/etc/x-ui/proxy.json`; бокс работает как сервис `x-ui`, запуская `x-ui proxy`, а `update.sh` сам определяет прокси-бокс и обновляет его в режиме прокси.

3. На реальной панели укажите подмену хоста на домен/IP прокси (пункт **а**).
