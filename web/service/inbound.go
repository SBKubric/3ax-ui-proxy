// Package service provides business logic services for the 3AX-UI web panel,
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	// A chain-following inbound takes the active edge's neighbour (ADR 0005).
	if err := (&ChainService{}).PrepareFollower(inbound); err != nil {
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	oldInbound.FollowChain = inbound.FollowChain
	if inbound.Listen == "" || inbound.Listen == "0.0.0.0" || inbound.Listen == "::" || inbound.Listen == "::0" {
		oldInbound.Tag = fmt.Sprintf("inbound-%v", inbound.Port)
	} else {
		oldInbound.Tag = fmt.Sprintf("inbound-%v:%v", inbound.Listen, inbound.Port)
