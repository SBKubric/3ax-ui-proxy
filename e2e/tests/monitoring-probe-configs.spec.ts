 ...
 * (docs/spec/proxy-chain.md §6.1, contract 3): a name the registry does not
 * have, or two different names, is 409 unknown_hop — never the proxy path a
 * mon-server could take for a hop. The joined-hop happy path needs a box to
 * join, which this harness has none of; monitoring-api.spec.ts walks a pending
 * hop, and the unit tests the rendering through a joined one.
 *
...
test.describe('probe configs per hop', () => {
  test('?hop= and ?edge= are 409 unknown_hop for a name the registry does not have', async ({ request }) => {
...
// Contract 3 (SBKubric/sane-3x-ui-monitoring#61): ensure takes monClientIds
// of 1–32 characters of [A-Za-z0-9_-] (they name AmneziaWG probe peers) and
// their paths, and answers with the pairs of mon-client × path left without a
// peer, `unallocated`. The compose panel has no AmneziaWG server, so nobody is
// left out.
test.describe('probe ensure, contract 3', () => {
  test('ensure answers contract 3 with unallocated and refuses a bad monClientId', async ({ request }) => {
    const output = xuiSetting('-monEnable', 'true', '-resetMonToken');
    const token = output.match(/^monToken: (.*)$/m)?.[1].trim();
    expect(token, `no token in:\n${output}`).toHaveLength(32);
    const headers = { Authorization: `Bearer ${token}` };
    ...
    try {
      const ok = await request.post('/mon/v1/probe/ensure', {
        ...
        data: {
          monClients: [
            { id: 'e2e-ams_1', name: 'e2e', region: 'NL', state: 'NEVER', lastHeartbeat: 0, paths: ['direct', 'hops'] },
          ],
        },
      });
      expect(ok.status()).toBe(200);
      expect(ok.headers()['x-mon-contract']).toBe('3');
